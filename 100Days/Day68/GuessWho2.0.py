...
def showImage():
  ...
  if person.strip().lower() == "charlotte":
    ...
    canvas.pack_forget() ## unpack the canvas if no image was found
    error.pack() ## display the error message
    return
  error.pack_forget() ## unpack the error message
  canvas.pack() ## only pack the canvas if an image was found
...
# add an error message if the image cannot be triangulated
error = tk.Label(text = "Triangulation failed. Try again.")

# add a button to initiate the image-finding subroutine
button = tk.Button(text = "Triangulate", command = showImage)
button.pack()

# create a canvas to the window
