...
def homepage():
  ...
  f = open("template/homepage.html", "r") ## load the template
...
def enterTheVoid():
  ...
  page = "" ## initialize a blank variable to store our HTML
  f = open("template/article.html", "r") ## load the template
  page = f.read() ## read the template's HTML into the `page` variable
  f.close()
...
@app.route('/FutureWorthFightingFor')
def future():
