...
# create base class
class character:
  def __init__(self, name, health, magic): ## initialize attributes
    self.name = name
    self.health = health
    self.magic = magic
...
class player(character):
  def __init__(self): ## no additional parameters needed because they're define below
    self.name = input("Name your player:\n")
    self.health = statGenerator()
    self.magic = statGenerator()
...
class enemy(character):
  ...
  def __init__(self, name, type): # must pass through arguments for these parameters upon instantiation because they should remain unique for each instance of the class
    self.name = name
    self.health = round(statGenerator() * 1.25)
    self.magic = round(statGenerator() * 1.25)
    self.type = type
    self.strength = round(statGenerator() * 1.25)
